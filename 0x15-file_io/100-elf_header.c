 ...
 *@e_ident: A pointer to an array containing the ELF class
 */
void print_type(unsigned in e_type, unsigned char *e_ident)
{
...
/**
 *print_entry - Prints the netry point of an ELF header
...
void print_entry(unsigned long int e_entry, unsigned char *e_indent)
{
	...
	{
		e_entry = ((e_entry << 8) & 0xFF00F00) | ((e_entry >> 8) & 0xFF00FF);
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	header = malloc(sizeof(Elf68_Ehdr));
	if (header == NULL)
	{
		close_elf(o);
		dprintf(STDERR_FILENO, "Error: Can't read file %s\n",  argv[1]);
